...
class Panoptic(JointsDataset):
    def __init__(self, model_name, root_id, max_people, num_views, color_rgb, dataset_dir, cam_list, \
        ...
        super().__init__(model_name, root_id, max_people, num_views, color_rgb, dataset_dir, cam_list, \
        ori_image_size, image_size, heatmap_size, \
        sigma, space_size, space_center, voxels_per_axis, individual_space_size, \
        is_train, transform)
        ...
        self.db_file = '{}_{}_cam{}_{}.pkl'.format(self.model_name, self.image_set, self.num_views, cam_str)
        self.db_file = osp.join(self.dataset_dir, self.db_file)

        logger.info(" ==============> Panoptic Dataset <============== ")
        logger.info(" ===> DB file path: {}".format(self.db_file))
        logger.info(" ===> Num of joints: {}".format(self.num_joints))
        logger.info(" ===> Num of views: {}".format(self.num_views))
        logger.info(" ===> Image set: {}".format(self.image_set))
        logger.info(" ===> Camera list: {}".format(self.cam_list))
        logger.info(" ===> Sequence list: {}".format(self.sequence_list))
        logger.info(" ===> Interval: {}".format(self._interval))
        ...
        logger.info(" ===> DB size: {}".format(self.db_size))
        logger.info("="*30)
    
    def _get_db(self):
        
        logger.info(" ===> Build {} DB file ...".format(self.image_set))
    ...
    def _get_cam(self):
        ...
        for seq in self.sequence_list:
            ...
            # elif self.model_name == 'voxelpose':
            #     for cam in calib["cameras"]:
            #         if (cam['panel'], cam['node']) in self.cam_list:
            #             sel_cam = {}
            #             sel_cam['K'] = np.array(cam['K'])
            ...
            for k, v in enumerate(cameras[seq]):
                ...
                our_cam['standard_T'] = v['t'] * 10.0 ## mvp
                our_cam['fx'] = v['K'][0, 0]
                our_cam['fy'] = v['K'][1, 1]
                our_cam['s'] = v['K'][0, 1]
                our_cam['cx'] = v['K'][0, 2]
    ...
    def __getitem__(self, idx):
        img_path, input, target, meta, input_heatmaps, target_heatmaps, target_weights, target_3d = super().__getitem__(idx)
        if self.model_name == 'faster_voxelpose':
            return input, target, meta, input_heatmaps
        elif self.model_name == 'voxelpose':
            return input, meta, input_heatmaps, target_heatmaps, target_weights, target_3d
