...
def affine_transform_pts_cuda_batch(pts_batch, t_batch):
    pts_homo_batch = torch.cat([
        pts_batch,
        torch.ones(pts_batch.shape[:-1]+(1,),
                   device=pts_batch.device)], dim=-1)
    out_batch = torch.matmul(pts_homo_batch, t_batch.transpose(2, 3))
    return out_batch