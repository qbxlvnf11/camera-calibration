...
def load_checkpoint(model, optimizer, output_dir, filename='checkpoint.pth.tar'):
    ...
    if os.path.isfile(file):
        ...
        for k in optimizer.keys():
            optimizer[k].load_state_dict(checkpoint[k+'_optimizer'])
            
        print('=> load checkpoint {} (epoch {})'.format(file, start_epoch))
        print(' ======== Load checkpoint ========')
        print(' ==> keys of checkpoint:', checkpoint.keys())
        print(' ==> len of weights:', len(checkpoint['state_dict'].keys()))
        print('='*30)
        print()
        print()
...
def save_checkpoint(states, is_best, optimizer, output_dir, filename='checkpoint.pth.tar'):
    
    for k in optimizer.keys():
        states[k+'_optimizer'] = optimizer[k].state_dict()
    
    print(' ======== Save checkpoint ========')
    print(' ==> keys of checkpoint:', states.keys())
    print(' ==> len of weights:', len(states['state_dict'].keys()))
    print('='*30)
    print()
    print()
...
def filter_backbone_keys(key):
    # Add your criteria here
    # For example, remove keys containing 'fc' or 'layer' in their names
    return 'backbone' not in key

def filter_model_keys(key):
    # Add your criteria here
    # For example, remove keys containing 'fc' or 'layer' in their names
    return 'backbone' in key